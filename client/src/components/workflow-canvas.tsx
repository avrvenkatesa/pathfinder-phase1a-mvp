    ...
    <div className="mt-4">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {recommendations.slice(0, showAll ? undefined : 3).map((contact, idx) => (
            <div
              key={contact.id}
              className="p-3 bg-white border rounded-lg hover:shadow-md transition-shadow relative"
            >
              {/* Compare Checkbox */}
              <div className="absolute top-3 right-3">
                <input
                  type="checkbox"
                  checked={compareList.some(c => c.id === contact.id)}
                  onChange={() => handleCompareToggle(contact)}
          ...
          {compareList.length >= 2 && (
            <button
              ...
              className="w-full mt-2 px-3 py-2 bg-purple-500 text-white rounded text-sm hover:bg-purple-600 transition-colors"
            >
      ...
      {showCompareModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[80vh] overflow-auto m-4">
            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold">Compare Contacts ({compareList.length})</h3>
              <button
                  ...
                  <div key={contact.id} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="mb-3">
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-bold text-blue-600">{contact.matchScore}%</span>
                      </div>
                    ...
                    {contact.matchedSkills.length > 0 && (
                      <div className="mb-2">
                        <h6 className="text-xs font-medium text-green-700 mb-1">Matched Skills</h6>
                        <div className="flex flex-wrap gap-1">
                          {contact.matchedSkills.slice(0, 3).map((skill: string, idx: number) => (
                            <span key={idx} className="px-1 py-0.5 bg-green-100 text-green-800 rounded text-xs">
                              {skill}
                            </span>
                          ))}
                          {contact.matchedSkills.length > 3 && (
                            <span className="text-xs text-gray-500">+{contact.matchedSkills.length - 3}</span>
                          )}
                        </div>
                      </div>
                    )}
