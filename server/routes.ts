...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(
    "/api/contacts/:id/certifications",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const certifications = await storage.getContactCertifications(
          req.params.id,
        );
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch contact certifications" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const certification =
          await storage.createContactCertification(certificationData);
        ...
        res
          .status(500)
          .json({ message: "Failed to create contact certification" });
      }
    },
  );
  ...
  app.put("/api/certifications/:id", isAuthenticated, async (req: any, res) => {
    try {
      const certificationData = updateContactCertificationSchema.parse(
        req.body,
      );
      const certification = await storage.updateContactCertification(
        req.params.id,
        certificationData,
      );
      ...
      res
        .status(500)
        .json({ message: "Failed to update contact certification" });
    }
  });
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete contact certification" });
      }
    },
  );
  ...
  app.get(
    "/api/contacts/:id/availability",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const availability = await storage.getContactAvailability(
          req.params.id,
        );
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch contact availability" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const availability =
          await storage.createContactAvailability(availabilityData);
        ...
        res
          .status(500)
          .json({ message: "Failed to create contact availability" });
      }
    },
  );
  ...
  app.put("/api/availability/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const availability = await storage.updateContactAvailability(
        req.params.id,
        availabilityData,
      );
      ...
      res
        .status(500)
        .json({ message: "Failed to update contact availability" });
    }
  });
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete contact availability" });
      }
    },
  );
  ...
  app.get("/api/contacts/available", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const workloadThreshold = req.query.threshold
        ? parseInt(req.query.threshold as string)
        : 80;

      const contacts = await storage.getAvailableContacts(
        userId,
        workloadThreshold,
      );
      res.json(contacts);
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to calculate contact capacity" });
      }
    },
  );

  app.get(
    "/api/contacts/assignment-candidates",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        if (skills.length === 0) {
          return res
            .status(400)
            .json({ message: "At least one required skill must be specified" });
        }
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch assignment candidates" });
      }
    },
  );
  ...
  app.post(
    "/api/contacts/assignment-recommendations",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        if (!requiredSkills || requiredSkills.length === 0) {
          return res
            .status(400)
            .json({ message: "Required skills must be specified" });
        }

        const recommendations =
          await contactService.getAssignmentRecommendations(
            requiredSkills,
            userId,
            options,
          );
        ...
        res
          .status(500)
          .json({ message: "Failed to generate assignment recommendations" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const result = await contactService.assignSkillsBulk(
          req.params.id,
          skills,
        );
        res.json(result);
  ...
  app.get(
    "/api/contacts/capacity-analysis",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        const analysis =
          await contactService.getCapacityOptimizationSuggestions(userId);
        res.json(analysis);
      } catch (error) {
        console.error("Error performing capacity analysis:", error);
        res
          .status(500)
          .json({ message: "Failed to perform capacity analysis" });
      }
    },
  );
  ...
  // Skills gap analysis
  app.post(
    "/api/contacts/skills-gap-analysis",
    isAuthenticated,
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        ...
        if (!requiredSkills || !Array.isArray(requiredSkills)) {
          return res
            .status(400)
            .json({ message: "Required skills array is required" });
        }

        const analysis = await contactService.analyzeSkillsGap(
          requiredSkills,
          userId,
        );
        res.json(analysis);
        ...
        res
          .status(500)
          .json({ message: "Failed to perform skills gap analysis" });
      }
    },
  );
  ...
  app.post("/api/relationships", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const relationship =
        await storage.createContactRelationship(relationshipData);
  ...
  app.put("/api/relationships/:id", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const relationshipData = { ...req.body, userId };
      const relationship = await storage.updateContactRelationship(
        req.params.id,
        relationshipData,
        userId,
      );
  ...
  app.delete(
    ...
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        const deleted = await storage.deleteContactRelationship(
          req.params.id,
          userId,
        );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const result = await storage.bulkUpdateRelationships(
          relationshipIds,
          operation,
          userId,
        );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        ...
        const assignments = await storage.getWorkflowAssignments(userId, {
          contactId,
          workflowName,
        });
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch workflow assignments" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        const assignment =
          await storage.createWorkflowAssignment(assignmentData);
        res.status(201).json(assignment);
      } catch (error) {
        console.error("Error creating workflow assignment:", error);
        res
          .status(500)
          .json({ message: "Failed to create workflow assignment" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to create bulk workflow assignments" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        const duplicated = await storage.duplicateWorkflow(
          req.params.id,
          userId,
        );
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        const instance = await storage.getWorkflowInstance(
          req.params.id,
          userId,
        );
        ...
        if (!instance) {
          return res
            .status(404)
            .json({ message: "Workflow instance not found" });
        }
  ...
  app.get(
    ...
    async (req: any, res) => {
      try {
        const userId = req.user.claims.sub;
        const instances = await storage.getWorkflowInstancesByWorkflow(
          req.params.id,
          userId,
        );
        res.json(instances);
